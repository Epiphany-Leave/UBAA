...
void main() {
  ...
  testWidgets('Android login preparation without credentials', (tester) async {
    await RustLib.init();
    ...
    try {
      ...
      for (final route in result.routes) {
        expect(route.state, BridgeRouteLoginState.ready,
            reason: '${route.route}: ${route.error?.code} ${route.error?.message}');
      }
