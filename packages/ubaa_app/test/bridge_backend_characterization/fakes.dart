...
class _CharacterizationBridgeClient implements BridgeClient {
  ...
  Future<BridgeRoutedJudgeSummaries> cachedJudgeAssignments({
    ...
    required bool refresh,
  }) => judgeAssignments(includeExpired: includeExpired);
  @override
  ...
  Future<BridgeRoutedSigninWeek> signinWeek({
    ...
    final result = await signinToday();
    return BridgeRoutedSigninWeek(
      data: [
        BridgeSigninDay(date: date, isFuture: false, classes: result.data),
      ],
      route: result.route,
    );
  }
